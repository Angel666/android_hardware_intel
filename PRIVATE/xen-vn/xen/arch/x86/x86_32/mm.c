...
#define CNT ((sizeof(*frame_table) & -sizeof(*frame_table)) / \
    ...
    if ( max_page * BYTES_PER_LONG  > XEN_MPT_PHYS_SIZE )
        panic("No enough memory for mpt, only support up to 1G system mem!\n");
    ...
    for ( i = 0; i < (mpt_size >> L2_PAGETABLE_SHIFT); i++ )
    {
        ...
        /* Hardcoded MPT table to physical addres to 4M and fixed size 2M */
        pg = maddr_to_page(XEN_MPT_PHYS_START << 20);
        ...
        memset((void *)(RDWR_MPT_VIRT_START + (i << L2_PAGETABLE_SHIFT)), 0x55,
               XEN_MPT_PHYS_SIZE);
        printk("MPT table initialization done, and locate @ :0x%lx\n", page_to_mfn(pg) << 12);
    }
#undef CNT
#undef MFN
