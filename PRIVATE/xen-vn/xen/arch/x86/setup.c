...
s8 __read_mostly xen_cpuidle = 0;
...
#else
        init_boot_pages(start, start + PAGE_ALIGN(initial_images[i].mod_end));
#endif
    }
...
extern void display_xen_heap_info(void);
...
#ifdef CONFIG_CTP
			/* 1-13M is always RAM in CTP Platform */
			if ( e820_raw[e820_raw_nr].addr == 0x100000 )
...
#define reloc_size ((__pa(&_end) + mask) & ~mask)
    ...
    xenheap_phys_end = XEN_HEAP_PHYS_END << 20;
#else
    if ( !xen_phys_start )
    ...
    for ( i = 0; i < boot_e820.nr_map; i++ )
    {
        ...
        e = (boot_e820.map[i].addr + boot_e820.map[i].size) & ~mask;
#if defined(CONFIG_X86_32)
        s = max_t(uint64_t, s, XEN_HV_PHYS_MEM_END << 20);
#else
    ...
    if (smp_found_config && sfi_disabled)
        get_smp_config();
