...
/************************************************************
 * PLATFORM TIMER 5: TIME STAMP COUNTER (TSC)
 */
...
static int __init init_tsc(struct platform_timesource *pts)
{
    if (!tsc_freq)
        return 0;
...
static struct platform_timesource __initdata plt_tsc =
{
    .id = "tsc",
    .name = "TSC",
    .read_counter = read_tsc_count,
    .counter_bits = 32, //FIXME: should we increase this?
    .init = init_tsc,
};
...
static void __init init_platform_timer(void)
{
    static struct platform_timesource * __initdata plt_timers[] = {
        &plt_cyclone, &plt_tsc, &plt_hpet, &plt_pmtimer, &plt_pit
    };
...
/* Early init function. */
u64 __init early_time_init_pit(void)
{
...
u64 __init early_time_init_mrst(void)
{
	u32 lo, hi, ratio, fsb;

	rdmsr(MSR_IA32_PERF_STATUS, lo, hi);
	printk("IA32 perf status is 0x%x, 0x%0x\n", lo, hi);
	ratio = (hi >> 8) & 0x1f;
	printk("ratio is %d\n", ratio);
	if (!ratio) {
		printk("read a zero ratio, should be incorrect!\n");
		printk("force tsc ratio to 16 ...\n");
		ratio = 16;
	}
	...
	case BSEL_SOC_FUSE_111:
		...
		break;
	default:
		printk("%s: unsupported BSEL_SOC_FUSE: %d, fall back to 83SKU",
	...
	if (boot_cpu_data.x86_model == 0x35
		&& fsb == FSB_FREQ_133SKU) {
		printk("See a Clovertrail+ B0/B1/B2 processor and will correct its tsc info!\n");
		/* The 133MHz FSB for CLVP+ is actuall 133.120 MHz */
		fsb = 133120;
	}

	tsc_freq = (u64)ratio * fsb * 1000;
	set_bit(X86_FEATURE_TSC_RELIABLE, boot_cpu_data.x86_capability);
...
void __init early_time_init(void)
{
    ...
    /* Medfield does not have a PIT/PIC */
    if (boot_cpu_data.x86 == 6 && (boot_cpu_data.x86_model == 0x27 || boot_cpu_data.x86_model == 0x35) )
        tmp = early_time_init_mrst();
    else
        tmp = early_time_init_pit();
