...
static long __init calibrate_APIC_pit(void)
{
    ...
    result = (tt1-tt2)*APIC_DIVISOR/LOOPS*HZ;

    if (cpu_has_tsc) {
        unsigned long cpu_freq = (t2-t1)/LOOPS*HZ;
...
static long __init calibrate_APIC_mrst(void)
{
	u32 lo, hi, fsb;
	...
	if (boot_cpu_data.x86_model == 0x35
		&& fsb == FSB_FREQ_133SKU) {
		printk("See a Clovertrail+ B0/B1/B2 processor and will correct its fsb info!\n");
		/* The 133MHz FSB for CLVP+ is actuall 133.120 MHz */
		fsb = 133120;
	}

	return (long)fsb * 1000;
}
...
static void __init calibrate_APIC_clock(void)
{
    ...
    if (boot_cpu_data.x86 == 6 && (boot_cpu_data.x86_model == 0x27 || boot_cpu_data.x86_model == 0x35))
      bus_freq = calibrate_APIC_mrst();
    else
      bus_freq = calibrate_APIC_pit();
