...
uint32_t compute_nr_boot_pages(void)
{
    int i;
    uint32_t nr_pages = 0;
...
bool_t assign_domain_boot_pages(struct domain *d, unsigned long mfn_base,
                        unsigned long nr_pages)
{
    int i;
    cpumask_t mask;
    struct page_info *pg = mfn_to_page(mfn_base);
...
unsigned long alloc_domain_boot_pages(struct domain *d, unsigned long nr_pfn,
                        unsigned long pfn_align)
{
    unsigned long pfn = alloc_boot_pages(nr_pfn, pfn_align);

    BUG_ON(pfn == 0);

    if (!assign_domain_boot_pages(d, pfn, nr_pfn))
        BUG();
    return pfn;
}

bool_t alloc_boot_pages_special(struct domain *d, unsigned long ps, unsigned long pe)
{
    struct bootmem_region *r = NULL;
    int i;

    printk("%s, start:%lx, end:%lx\n", __func__, ps, pe);
    if ( ps > pe ) {
        printk("Error: can't allocate the boot pages for Xen!, start pfn:%lx,\
            end pfn:%lx\n", ps, pe);
        return 0;
    }
    ...
    /* Hack: we assume dom0 needs contiguous memory. */
    if ( i < 0 ) {
        printk("Error: boot pages allocation failure! start:%lx, end:%lx\n", ps, pe);
        return 0;
    }


    if ( unlikely(ps == r->s && pe == r->e) ) {
        memmove(&bootmem_region_list[i], &bootmem_region_list[i+1],
            (nr_bootmem_regions - i) * sizeof(*bootmem_region_list));
        nr_bootmem_regions--;
    }
    else if ( ps == r->s )
       ...
       r->e = ps;
    else if (ps > r->s && pe < r->e){
        r->e = ps;
        bootmem_region_add(pe, r->e);
    } else
         BUG();

    if ( !assign_domain_boot_pages(d, ps, pe -ps ) )
       BUG();
...
unsigned long
alloc_one_bootmem_region(struct domain *d, unsigned long *pnr_pfns)
{
    struct bootmem_region *r;
    int i, nr_pages;
    ...
    nr_pages = r->e - r->s;
    r->e = r->s;
    nr_bootmem_regions = i;

    if ( !assign_domain_boot_pages(d, r->s, nr_pages) )
        BUG();
    *pnr_pfns = nr_pages;
    printk("%s: start:%lx, nr:%x\n", __func__, r->s, nr_pages);
    ...
    struct domain *d, unsigned int order, unsigned int memflags)
{
    ...
    printk("Allocate pages from domain heap, order:%d\n", order);
    BUG_ON(d != dom0);
    va = alloc_xenheap_pages(order, memflags);
    if (!va) {
        printk("Error: Don't have enough memory in Xen heap!\n");
        return NULL;
    }
    pg = virt_to_page(va);
...
void display_xen_heap_info(void)
{
    pagealloc_info(1);
}
