...
static DEFINE_PER_CPU(int, mwait_loop);
static DEFINE_PER_CPU(int, monitor_variable);
#define C6_HINT 0x52

long do_ipi(unsigned long cpu);
...
long do_vcpu_op(int cmd, int vcpuid, XEN_GUEST_HANDLE(void) arg)
{
    ...
    case VCPUOP_up:
        ...
        if ( test_and_clear_bit(_VPF_down, &v->pause_flags) ) {
            vcpu_wake(v);
            /* Issue real IPI to wake up physical CPU. */
            do_ipi(v->vcpu_id);
        }
    ...
    case VCPUOP_down:
    {
        this_cpu(mwait_loop) = 1;
        if ( !test_and_set_bit(_VPF_down, &v->pause_flags) )
            vcpu_sleep_nosync(v);

        while ( this_cpu(mwait_loop) )
            do_mwait_suspend(C6_HINT, 1, &this_cpu(monitor_variable));
        break;
    }
...
long do_mwait_suspend(unsigned long eax, unsigned long ecx,
              void *monitor_addr)
{
    ...
    BUG_ON(!vcpu_info(current, evtchn_upcall_mask));

    if ( (cpu == 0) &&
         (vcpu_info(current, evtchn_upcall_pending) || softirq_pending(cpu)) )
        goto exit;
...
long do_mwait_idle(unsigned long eax, unsigned long ecx,
              void *monitor_addr, unsigned long resched_addr)
{
    ...
    if ( (vcpu_info(current, evtchn_upcall_pending) || softirq_pending(cpu)) )
        goto exit;

    gmfn = resched_addr >> PAGE_SHIFT;
    mfn = gmfn_to_mfn(current->domain, gmfn);
    va = map_domain_page(mfn);
    va = (void *)((unsigned long)va + (resched_addr & ~PAGE_MASK));

    __monitor(monitor_addr, 0, 0);
    smp_mb();
    if ( !test_bit(TIF_NEED_RESCHED, (u32 *)va) )
        __mwait(eax, ecx);
...
long do_ipi(unsigned long cpu)
{
    per_cpu(mwait_loop, cpu) = 0;
    smp_mb();
    /*
     * Issue both IPI and monitor address touching to ensure BSP can wake up AP.
     */
    send_IPI_mask(cpumask_of(cpu), EVENT_CHECK_VECTOR);
    per_cpu(monitor_variable, cpu) = ~0;
    return 0;
}

long do_raise_nmi(unsigned long cpu)
{
	struct domain *d = current->domain;

	return send_guest_trap(d, cpu, TRAP_nmi);
