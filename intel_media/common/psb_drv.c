...
void mrst_get_fuse_settings(struct drm_device *dev)
{
	...
	if (IS_MDFLD(dev)) {
#if defined(KSEL_BYPASS_83_100_ENABLE) && KSEL_BYPASS_83_100_ENABLE
		dev_priv->ksel = KSEL_BYPASS_83_100;
#endif /* KSEL_BYPASS_83_100_ENABLE */

#if defined(KSEL_CRYSTAL_19_ENABLED) && KSEL_CRYSTAL_19_ENABLED
		dev_priv->ksel = KSEL_CRYSTAL_19;
#endif /*  KSEL_CRYSTAL_19_ENABLED */

#if defined(KSEL_CRYSTAL_38_ENABLED) && KSEL_CRYSTAL_38_ENABLED
...
#if CONFIG_PANEL_LVDS_BRIDGE
	/* FIXME:
	 * This is a workaround for Santos10 project. Usually panel ID
	 * should be detected from IA FW dynamically, befor FW's ready,
	 * workaround it first. This MUST be reverted if FW's ready.
	 */
	DRM_INFO("%s: WORKAROUND: changes panel name from '%s' to 'tc358764'\n",
			__func__, dev_priv->panel_info.name);
	strcpy(dev_priv->panel_info.name, "tc358764");
	dev_priv->panel_info.mode = MDFLD_DSI_ENCODER_DPI;
#endif
...
static int psb_driver_load(struct drm_device *dev, unsigned long chipset)
{
	...
	*/
	...
	dev_priv->pipestat[0] = PSB_RVDC32(PIPEASTAT);
	dev_priv->pipestat[1] = PSB_RVDC32(PIPEBSTAT);
	dev_priv->pipestat[2] = PSB_RVDC32(PIPECSTAT);
...
void psb_flip_abnormal_debug_info(struct drm_device *dev)
{
	...
	for (pipe = 0; pipe < PSB_NUM_PIPE; pipe++) {
		...
		if (nanosec_rem > 200000000) {
			DRM_INFO("pipe %d vsync te missing %ldms !\n\n",
				 pipe, nanosec_rem/1000000);
			dev_priv->vsync_te_working[pipe] = false;
			if (pipe == 0)
			{
				atomic_set(&dev_priv->mipi_flip_abnormal, 1);
				mode = get_panel_mode(dev);
				if (mode == MDFLD_DSI_ENCODER_DPI) {
					dev_priv->vsync_missing_cnt++;
					if (dev_priv->vsync_missing_cnt > 3) {
